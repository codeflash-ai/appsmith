...
const getEntityNameCache: WeakMap<
  DataTreeEntity,
  { config: DataTreeEntityConfig | undefined; name: string | undefined }
> = new WeakMap();
...
export function getEntityName(
  ...
  // Only cache for non-null objects; primitives or null/undefined are handled directly
  // to preserve original behavior and avoid WeakMap errors.
  if (entity && typeof entity === "object") {
    const cached = getEntityNameCache.get(entity);
    if (cached && cached.config === entityConfig) {
      return cached.name;
    }
    ...
    getEntityNameCache.set(entity, { config: entityConfig, name });
    return name;
  }

  // Fallback for primitives/null/undefined - preserve original control flow and errors.
